...
Keyball GesturesはLinuxおよびmacOS環境でトラックパッドライクなマウスジェスチャーを実現するツールです。Keyballのトラックボールとキーの組み合わせで、直感的なワークスペース操作を可能にします。
...
- **OS**: 
  ...
  - macOS: macOS 10.9以降（アクセシビリティ権限が必要）
- **必須デバイス**: Keyball
...
設定例：
- Linux: `example-config.toml`
- macOS: `example-config-macos.toml`
...
- **macOS**:
  - **アクセシビリティ権限**: キーボードとマウスイベントを監視するため、システム環境設定でアクセシビリティ権限を付与する必要があります。
  - **CoreGraphics API**: macOSのCoreGraphics Event APIを使用してトラックパッドジェスチャーをシミュレートします。
...
#### アクセシビリティ権限
初回実行時に以下のエラーが表示される場合：
```
タッチパッドの初期化に失敗しました。アクセシビリティ権限を確認してください
```

解決方法：
1. システム環境設定 > セキュリティとプライバシー > プライバシー > アクセシビリティ を開く
2. 左下の鍵アイコンをクリックして変更を許可
3. keyball-gestures または Terminal.app にチェックを入れる

#### スクロールが動作しない
- F13/F14キーが正しく認識されているか確認（`test-macos`プログラムで確認可能）
- Keyballのファームウェアが最新版であることを確認
- 設定ファイルのキーコードが正しいか確認
...
解決方法：
- `sudo`で実行するか、`input`グループに所属していることを確認
- udevルールが正しく設定されているか確認

### 共通

#### デバイスが認識されない
- Keyballが正しく接続されているか確認
- `keyball-gestures --api`でWeb UIを開き、デバイス一覧を確認
- 設定ファイルで優先デバイス名を指定してみる
EOF < /dev/null